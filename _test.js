...
async function loadH2H(){
  var a = document.getElementById('h2h-a');
  var b = document.getElementById('h2h-b');
  var ta = a ? a.value : allTeams[0];
  var tb = b ? b.value : (allTeams[1] || allTeams[0]);
  try{
    ...
    document.getElementById('h2h-result').innerHTML =
      '<div class="h2h-compare">' +
        '<div class="h2h-team"><div class="flag">' + getFlag(d.team_a) + '</div><div class="name">' + d.team_a + '</div><div class="elo">Elo评分：' + (d.elo_a||'暂无') + '</div></div>' +
        '<div class="h2h-vs">对阵</div>' +
        '<div class="h2h-team"><div class="flag">' + getFlag(d.team_b) + '</div><div class="name">' + d.team_b + '</div><div class="elo">Elo评分：' + (d.elo_b||'暂无') + '</div></div>' +
      '</div>' +
      '<div class="h2h-probs">' +
        '<div><div class="p">' + ((d.win_prob_a||0).toFixed(1)) + '%</div><div class="l">' + d.team_a + ' 获胜</div></div>' +
        '<div><div class="p">' + ((d.draw_prob||0).toFixed(1)) + '%</div><div class="l">平局</div></div>' +
        '<div><div class="p">' + ((d.win_prob_b||0).toFixed(1)) + '%</div><div class="l">' + d.team_b + ' 获胜</div></div>' +
      '</div>' +
      '<div class="text-center mt-8" style="font-size:.75em;color:#7aa4c8;">预期进球：' + cn(d.team_a) + ' ' + (d.expected_goals_a||'暂无') + ' — ' + (d.expected_goals_b||'暂无') + ' ' + cn(d.team_b) + '</div>';
  }catch(e){
...
function renderBracketTree(){
  ...
  var rounds = ['R32','R16','QF','SF','3rd','F'];
  var labels = {R32:'32强',R16:'16强',QF:'四分之一决赛',SF:'半决赛','3rd':'季军赛',F:'🏆 决赛'};
  var counts = {R32:16,R16:8,QF:4,SF:2,'3rd':1,F:1};
  var h = '<div class="bracket-wrap"><div class="bracket-tree">';
  for(var ri=0; ri<rounds.length; ri++){
    var rnd = rounds[ri];
    var ms = matchData.matches.filter(function(m){ return m.round === rnd; }).sort(function(a,b){ return a.id.localeCompare(b.id); });
    var isFinal = rnd === 'F' || rnd === '3rd';
    h += '<div class="bracket-col" style="min-width:' + (isFinal?190:165) + 'px">';
    h += '<div class="col-title">' + labels[rnd] + '</div>';
    h += '<div style="display:flex;flex-direction:column;justify-content:space-around;min-height:' + Math.max(280, ms.length*60) + 'px">';
    for(var i=0; i<ms.length; i++){
      var m = ms[i];
      var hw = m.winner === 'home', aw = m.winner === 'away';
      var hs = m.home_score !== null && m.home_score !== undefined;
      h += '<div class="bracket-match' + (m.winner?' won':'') + '" onclick="openModal(\'' + m.id + '\')" title="点击编辑比分">';
      h += '<div class="row top' + (hw?' w':'') + (!m.home?' tbd':'') + '"><span>' + (m.home ? cn(m.home) : '待定') + '</span><span class="sc">' + (hs?m.home_score:'-') + '</span></div>';
      h += '<div class="row' + (aw?' w':'') + (!m.away?' tbd':'') + '"><span>' + (m.away ? cn(m.away) : '待定') + '</span><span class="sc">' + (hs?m.away_score:'-') + '</span></div>';
      if(m.date) h += '<div style="font-size:.6em;color:#7aa4c8;text-align:center;padding:2px">📅 ' + m.date + ' 📍 ' + (m.venue||'') + '</div>';
      h += '</div>';
    }
    h += '</div></div>';
    if(ri < rounds.length - 1){
      h += '<div class="bracket-arrow"><svg width="40" height="' + Math.max(320, counts[rnd]*65) + '"><g stroke="#c9a84c" stroke-width="2.5" fill="none">';
      for(var i=0; i<ms.length; i+=2){
        if(i+1 >= ms.length) break;
        var y1 = 35 + i*65, y2 = y1 + 65, ym = (y1+y2)/2;
        h += '<path d="M0 ' + y1 + ' L18 ' + y1 + ' L18 ' + ym + ' L36 ' + ym + '"/><path d="M0 ' + y2 + ' L18 ' + y2 + ' L18 ' + ym + '"/>';
      }
      h += '</g></svg></div>';
    }
  }
